...
public:
    ...
    FFTXProblem(const std::vector<int>& sizes1, std::string name1) {  
     ...
     FFTXProblem(std::vector<void*>& args1, std::vector<int>& sizes1, std::string name1) {
        args = args1;   
        sizes = sizes1;
        name = name1;
    }
...
void FFTXProblem::transformSPIRAL(){
...
void FFTXProblem::run(Executor e) {
    ...
    std::cout << "we are leaving spiral now" << std::endl;
    #endif
}
