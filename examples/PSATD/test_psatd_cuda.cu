...
void Resample(const std::vector<int>& out = {1, 2, 3}, 
    ...
    extractKernel<<<gridDim, blockDim>>>(input, d_output, in.at(0), in.at(1), in.at(2), out.at(0), out.at(1), out.at(2));
    // print1(temp.data(), "temp");
    
    cufftDoubleComplex* fft_out;
    cudaMalloc(&fft_out, out.at(0)*out.at(1)*((out.at(2)/2)+1) *sizeof(cufftDoubleComplex));
    cufftHandle plan;
    ...
    int3 dims = make_int3(out.at(0), out.at(1), out.at(2));
  ...
  //  fftw_execute(ip1);
    ...
    cufftExecZ2D(plan2, fft_out, (cufftDoubleReal *)output);
