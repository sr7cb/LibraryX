#ifndef FFTX_ACCELBACKEND_HEADER
#define FFTX_ACCELBACKEND_HEADER
...
if ( DEFINED ENV{FFTX_ACCEL_HOME} )
    set ( FFTX_ACCEL_DIR $ENV{FFTX_ACCEL_HOME} )
else ()
    if ( "x${FFTX_ACCEL_HOME}" STREQUAL "x" )
        message ( FATAL_ERROR "FFTX_ACCEL_HOME environment variable undefined and not specified on command line" )
    endif ()
    set ( FFTX_ACCEL_DIR ${FFTX_ACCEL_HOME} )
endif ()
...
float Executor::initAndLaunch(std::vector<void*>& args, std::string name) {
    ...
    std::string destroy = oss2.str();
        ...
        void (*fn2)(float *, float *, ...) = (void (*)(float *, float *, ...))dlsym(shared_lib, transform.c_str());
    ...
    if(fn2) {
      if(args.size() < 3)
        fn2((float*)args.at(0),(float*)args.at(1));
      else
        fn2((float*)args.at(0),(float*)args.at(1), (float*)args.at(2));
    }else {
...
void Executor::execute(std::string result) {
    ...
    #else
        systemret = system("cmake -DCMAKE_BUILD_TYPE=Debug -DCMAKE_VERBOSE_MAKEFILE=ON . && make"); 
    #endif
