...
typedef BoxData<MyDouble> Scalar;
typedef BoxData<MyDouble, NUMCOMPS> Vector;
...
f_thresholdF(
             Var<short>& a_tags,
             Var<MyDouble, NUMCOMPS>& a_U)
{
  MyDouble thresh = 1.001;
...
void f_upwindState_(
        ...
        MyDouble                       a_gamma)
{
    const MyDouble& rhol = a_low(0);
    const MyDouble& rhor = a_high(0);
    const MyDouble& ul = a_low(a_dir+1);
    const MyDouble& ur = a_high(a_dir+1);
    const MyDouble& pl = a_low(NUMCOMPS-1);
    const MyDouble& pr = a_high(NUMCOMPS-1);
    MyDouble gamma = a_gamma;
    MyDouble rhobar = (rhol + rhor)*.5;
    MyDouble pbar = (pl + pr)*.5;
    MyDouble ubar = (ul + ur)*.5;
    MyDouble cbar = sqrt(gamma*pbar/rhobar);
    MyDouble pstar = (pl + pr)*.5 + rhobar*cbar*(ul - ur)*.5;
    MyDouble ustar = (ul + ur)*.5 + (pl - pr)/(2*rhobar*cbar);
    int sign;
    if (ustar > 0) 
    {
        sign = -1;
    ...
    MyDouble outval = a_out(0) + (pstar - a_out(NUMCOMPS-1))/(cbar*cbar);
    if (cbar + sign*ubar > 0)
    {
        a_out(0) = outval;
        a_out(a_dir+1) = ustar;
...
template<typename T, MemType MEM>
PROTO_KERNEL_START
void f_waveSpeedBound_(Var<MyDouble,1>& a_speed,
        const Var<T, NUMCOMPS, MEM>& a_W,
        MyDouble       a_gamma)
{
...
class BoxOp_Euler : public BoxOp<T, NUMCOMPS, 1, MEM>
{
    ...
    inline void init()
    {
        ...
        {
            m_interp_H[dir] = Stencil<MyDouble>::CellToFaceH(dir);
            m_interp_L[dir] = Stencil<MyDouble>::CellToFaceL(dir);
            m_divergence[dir] = Stencil<MyDouble>::FluxDivergence(dir);
            m_laplacian_f[dir] = Stencil<MyDouble>::LaplacianFace(dir);
        }
    }
            ...
            int a_dir) const
    {
        ...
        Vector W_ave_f = forall<MyDouble,NUMCOMPS>(f_upwindState, W_ave_L, W_ave_H, a_dir, gamma);
#if DIM>1
        Vector F_bar_f = forall<MyDouble,NUMCOMPS>(f_getFlux, W_ave_f, a_dir,  gamma);
...
private:
  ...
  //Array<std::shared_ptr<Array<BoxData<MyDouble>,DIM>>,DIM> m_data;
  //std::shared_ptr<BoxData<MyDouble, 1>> m_data;
  BoxData<MyDouble> m_data;
  Stencil<MyDouble> m_interp_H[DIM];
  Stencil<MyDouble> m_interp_L[DIM];
  Stencil<MyDouble> m_divergence[DIM];
  Stencil<MyDouble> m_laplacian_f[DIM];
};
